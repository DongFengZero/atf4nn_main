...
model = onnx.load('./temp_model/gemma/model.onnx')
nodes_to_remove = []
replace_map = {}
...
onnx.save(model, './temp_model/gemma/model.onnx')

feed_dict = {'input_ids': input_ids.numpy(), 'attention_mask': attention_mask.numpy()}
np.savez('./temp_model/gemma/inputs.npz', **feed_dict)
