...
model = onnx.load('./temp_model/chatglm/model.onnx')
nodes_to_remove = []
replace_map = {}
...
onnx.save(model, './temp_model/chatglm/model.onnx')

feed_dict = {'input_ids': input_ids.numpy(), 'attention_mask': attention_mask.numpy()}
np.savez('./temp_model/chatglm/inputs.npz', **feed_dict)
