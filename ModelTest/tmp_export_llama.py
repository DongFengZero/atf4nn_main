...
config = AutoConfig.from_pretrained('openlm-research/open_llama_3b', trust_remote_code=True)
...
model = onnx.load('./temp_model/llama/model.onnx')
nodes_to_remove = []
replace_map = {}
...
onnx.save(model, './temp_model/llama/model.onnx')

feed_dict = {'input_ids': input_ids.numpy(), 'attention_mask': attention_mask.numpy()}
np.savez('./temp_model/llama/inputs.npz', **feed_dict)
