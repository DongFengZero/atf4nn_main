...
class DefaultPolicy:
    ...
    def _assign_block_size(self, node: IRNode, td: TileDict, block_size: int):
        ...
        def _score(node, block_tile, td): # small is better
            # shape = node.propogate_inputs(block_tile)
            op_shape = node.propogate(block_tile)
            ...
            t_graph_compute = PartitionCompute(td, block_tile, self.arch.reg_cap, [node], self.arch, shape_map2)
            t_graph_compute.update_capacity_origin(self.arch.warp_size)
            t_graph_compute.update_map(shape_map)
            num_wave = max(np.ceil(np.prod(n_thread) / self.arch.warp_size), 1)
