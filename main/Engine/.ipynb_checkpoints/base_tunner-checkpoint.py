...
def subgraph_hash(output_nodes):
    ...
    #num_hash.append(hash(str(len(t_nodes))))
    graph_sig = hash(tuple(node_hash + edge_hash))
    return graph_sig

# def subgraph_hash(output_nodes, t_nodes):
#     nodes = find_topo_sort(output_nodes)
#     all_nodes = find_topo_sort(t_nodes)
#     node_hash, edge_hash, node2_hash = [], [], []
#     node_idx = {node.name : i for i, node in enumerate(nodes)}
#     for node in all_nodes:
#         if node.get_tag("skip") or node.is_placeholder() or node.is_output():
#             continue
#         else:
#             code = node.get_ir()
#             hex = hashlib.sha1(bytes(code, encoding="utf-8")).hexdigest()
#             node2_hash.append(int(hex, 16))
#     for node in nodes:
#         code = node.get_ir()
#         hex = hashlib.sha1(bytes(code, encoding="utf-8")).hexdigest()
#         node_hash.append(int(hex, 16))
#         if node in nodes:
#             for edge in node.outputs:
#                 sig = (node_idx[edge.src_node.name], node_idx[edge.dst_node.name], edge.src_id, edge.dst_id)
#                 edge_hash.append(hash(sig))
#     graph_sig = hash(tuple(node_hash + edge_hash + node2_hash))
#     return graph_sig
...
class Tunner(object):
    def __init__(self, arch,
                 device="cuda:0", check=False, topk=10, graph_rein = False) -> None:
        self._cache =  {}
        self._cache_name = {}
        self.device = device
        self.check = check
    ...
    def generate_configs(self, policy_list, output_nodes):
        ...
        for policy in policy_list:
            ...
            if remaining <= 0:
                break
            t_configs = policy(output_nodes, self.arch).emit_config(remaining)
            configs.extend(t_configs)
        return configs
