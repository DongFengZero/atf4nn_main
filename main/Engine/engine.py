...
class Engine:
    ...
    def _update_node_to_index_map(self, map, num_of_nodes):
        self.node_to_index = map
        self.num_of_node = num_of_nodes
        self.edge_weight_matrix = 1e8*torch.ones((self.num_of_node, self.num_of_node)).cuda()


    def _build_fusion_group(self, top_node, max_fusing_nodes):
        cur_group = [top_node]
        ...
        cp_result = None
        detect_flag = True
        while len(queue) > 0 and detect_flag:
            ...
            for edge in node.outputs:
                ...
                if len(cur_group) + len(fusing_nodes) < max_fusing_nodes + 1:
            ...
            if len(fusing_nodes) == 0 or len(cur_group) + len(fusing_nodes) > max_fusing_nodes: # too many dependency
                continue
            ...
            # print("kernel_name:","Group"+str(cur_group_id))
            # print("gain:",gain)
