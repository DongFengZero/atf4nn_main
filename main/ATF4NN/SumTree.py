...
class SumTree:
    ...
    def add(self, p, data):
        ...
        data_idx = self.data_pointer  # 【新增】记录数据索引
        ...
        return data_idx  # 【修改】返回索引
    
    def replace_at(self, data_idx, p, data):
        """
        【新增】在指定位置替换数据
        
        Args:
            data_idx: 数据数组中的索引 [0, capacity-1]
            p: 新的优先级
            data: 新的数据
        """
        if data_idx < 0 or data_idx >= self.capacity:
            raise ValueError(f"Invalid data_idx: {data_idx}")
    ...
    def get_priority_at(self, data_idx):
        """
        【新增】获取指定位置的优先级
        """
        tree_idx = data_idx + self.capacity - 1
